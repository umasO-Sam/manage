                                ...
                                <tbody class="divide-y divide-slate-100">
                                    @foreach ($staffList as $staff)
                                        ...
                                        <tr class="hover:bg-slate-50 {{ $canEditRow ? '' : 'bg-slate-50/60' }}">
                                            ...
                                            <td class="p-2.5">
                                                <span x-show="!editMode" class="font-mono">@days($staff->paid_leave_granted_current_year)</span>
                                                <input x-show="editMode" x-cloak @disabled(! $canEditRow) type="number" step="0.25" min="0" max="99.99" name="updates[{{ $staff->id }}][paid_leave_granted_current_year]"
                                                       value="{{ \App\Support\LeaveDays::format($staff->paid_leave_granted_current_year) }}" class="w-20 text-xs border rounded px-1.5 py-1 border-slate-300">
                                            </td>
                                            <td class="p-2.5">
                                                <span x-show="!editMode" class="font-mono">@days($staff->paid_leave_granted_last_year)</span>
                                                <input x-show="editMode" x-cloak @disabled(! $canEditRow) type="number" step="0.25" min="0" max="99.99" name="updates[{{ $staff->id }}][paid_leave_granted_last_year]"
                                                       value="{{ \App\Support\LeaveDays::format($staff->paid_leave_granted_last_year) }}" class="w-20 text-xs border rounded px-1.5 py-1 border-slate-300">
                                            </td>
                                            <td class="p-2.5 font-mono text-slate-500">@days($paidLeaveBalances[$staff->id]['remainingTotal'])</td>
                                            <td class="p-2.5">
                                                <div class="flex gap-2">
                                                    @if ($canEditRow)
