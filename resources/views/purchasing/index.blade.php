        ...
        <div class="max-w-[1800px] mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
                    ...
                    <div class="flex gap-3">
                        ...
                        @if (Auth::user()->is_procurement_manager)
                            ...
                            <button type="button" x-show="!editMode" @click="toggleEditMode()"
                                    class="text-sm font-semibold rounded-lg py-2 px-6 transition-colors border bg-white border-slate-300 text-slate-700 hover:bg-slate-50">
                                直接編集
                            </button>
                            <span x-show="editMode" x-cloak
                                    class="text-sm font-semibold rounded-lg py-2 px-6 border bg-amber-100 border-amber-300 text-amber-800">直接編集中</span>
            ...
            @if (Auth::user()->is_procurement_manager)
                <div x-show="editMode" x-cloak class="sticky top-2 z-10 bg-white border border-amber-200 rounded-xl p-3 shadow-sm flex flex-wrap justify-between items-center gap-2">
                    <span class="text-xs text-amber-700 font-semibold">直接編集モード: 表示中のレコードのセルを編集し、「編集を保存」を押してください。</span>
