                ...
                <table class="w-full text-left border-collapse whitespace-nowrap">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-200 text-xs font-semibold text-slate-600">
                            <th class="px-4 py-2.5">注番</th>
                            <th class="px-4 py-2.5">工事名</th>
                            <th class="px-4 py-2.5 text-center" title="依頼・作業日報・休暇申請の注番プルダウンに出すかどうか">プルダウン</th>
                            <th class="px-4 py-2.5">種別</th>
                            <th class="px-4 py-2.5">利用件数</th>
                    ...
                    <tbody class="divide-y divide-slate-100 text-sm">
                        @foreach ($orderNumbers as $orderNumber)
                            <tr class="hover:bg-slate-50">
                                <td class="px-4 py-1.5 font-semibold text-slate-800 {{ $orderNumber->matchesStandardFormat() ? 'font-mono' : '' }}">
                                    {{ $orderNumber->code }}
                                ...
                                <td class="px-4 py-1.5">
                                    <input form="order-number-{{ $orderNumber->id }}" type="text" name="project_name"
                                           ...
                                           class="text-xs border rounded-lg px-2 py-1 border-slate-300 w-full min-w-[12rem]">
                                </td>
                                ...
                                <td class="px-4 py-1.5">
                                    ...
                                    @elseif (! $orderNumber->matchesStandardFormat())
                                        <span class="text-[11px] font-bold px-2.5 py-0.5 rounded-full bg-amber-50 text-amber-700">自由入力</span>
                                    @else
                                        <span class="text-[11px] font-bold px-2.5 py-0.5 rounded-full bg-blue-50 text-blue-700">登録済み</span>
                                    @endif
                                </td>
                                <td class="px-4 py-1.5 text-slate-600">{{ $orderNumber->cards_count }} 件</td>
                                <td class="px-4 py-1.5">
