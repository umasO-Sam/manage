        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-3">
                ...
                <div class="flex justify-end gap-2">
                    ...
                    @if ($canDeleteCard)
                        {{-- confirm()の文言に改行を入れるとonsubmitが構文エラーになり、
                             確認なしで送信されてしまう。1行で書くこと。 --}}
                        <form method="POST" action="{{ route('projects.destroy', $card) }}"
                              onsubmit="return confirm('{{ $order->order_no }}／{{ $order->product_name }} を削除します。受注金額も原価計算・見積補助の集計から外れます。元に戻せません。よろしいですか？');">
                            @csrf
                            @method('DELETE')
