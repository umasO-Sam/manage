...
- テスト: `php artisan test`（現在 **588件パス**）。修正したら必ず通してからコミットする
...
| 勤怠管理者 | 承認済み申請の取消の**反映判断**、**休日勤務申請の最終承認**（9章）、**作業日報の代理入力**（10章）。「勤怠管理者の確認」画面とバッジ |
...
| 本人が日報を出せない（入院・退職・PCが無い等） | 勤怠管理者が作業日報画面の「代理入力する担当者」で本人を選んで代理提出する（10章） |
| 代理提出した日報を差し戻したのに本人が知らない | 仕様（10章）。代理提出分の差し戻しは**本人ではなく代理提出した勤怠管理者**に返る。代理提出者の作業日報画面の上部に一覧が出る |
