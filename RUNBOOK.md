...
- テスト: `php artisan test`（現在 **635件パス**）。修正したら必ず通してからコミットする
...
| 差し戻した人工がどこにも出てこない | 人工レコード確認の一覧の下の「差し戻し ○件」の枠（2026-08-17）。確定していないので上の一覧には出ない。この枠で修正・削除でき、「日報」でその日の作業日報確認へ飛べる |
...
- **差し戻しても人工レコードは消えない**。未確認（`is_provisional = true`）のまま残り、原価計算には
  ...
  **人工レコード確認の一覧の下に「差し戻し ○件」の別枠**を出して拾えるようにしている（2026-08-17）。
  仕入管理から入れた分は本人が日報を出し直しても直らないので、この枠で直接**修正・削除**する
  （「日報」ボタンでその日の作業日報確認へ飛べる）。**修正しても未確認のまま**で、確定するのは
  作業日報確認だけ。ここで確定にしてしまうと、誰も内容を見ないまま原価計算に乗ってしまう
