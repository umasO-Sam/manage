...
- テスト: `php artisan test`（現在 **649件パス**）。修正したら必ず通してからコミットする
...
| 注文書の項目が枠内で切れる | 仕様。1セル2行までで打ち切る。幅は 注番12文字／機械装置No19文字／メーカー全角6文字／品名全角8文字／数量4桁+単位 が1行に収まるよう実測して決めている。型式等が残り幅 |
