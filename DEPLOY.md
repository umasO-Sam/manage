...
# Xserverのメールアカウント(例: no-reply@saito-koken.co.jp)をサーバーパネルで
# 事前に作成し、そのSMTP情報を設定する
MAIL_MAILER=smtp
MAIL_HOST=<契約サーバーのホスト名>.xserver.jp
MAIL_PORT=587
MAIL_USERNAME=no-reply@saito-koken.co.jp
MAIL_PASSWORD=<メールアカウントのパスワード>
MAIL_ENCRYPTION=tls
MAIL_FROM_ADDRESS="no-reply@saito-koken.co.jp"
...
# 4. マイグレーション + 初期データ投入（担当者・ワークフロー・注番の初期値）
...
npm install
npm run build
# 無ければ、ローカルで `npm run build` した public/build/ フォルダをそのままアップロードすればOK（このステップは不要）
...
サーバーパネル「Cron設定」で以下を追加（**1分ごと**）:

```
php /home/<アカウント>/manage/artisan schedule:run >> /dev/null 2>&1
```
...
サーバーパネル「SSL設定」→ 対象ドメインで無料独自SSLを有効化。
反映後、`.env` の `APP_URL` が `https://` になっていることを再確認。
...
composer install --no-dev --optimize-autoloader
php artisan migrate --force
npm run build   # フロント変更があれば（またはローカルビルドを再アップロード）
php artisan config:cache
php artisan route:cache
php artisan view:cache
```
